...
func (engine *Engine) ShowSQL(show ...bool) {
	...
	if len(show) == 0 {
		engine.TLogger.SQL.Disabled = true
	} else {
		engine.TLogger.SQL.Disabled = show[0]
	}
}
...
func (engine *Engine) ShowExecTime(show ...bool) {
	if len(show) == 0 {
		engine.TLogger.ETime.Disabled = true
	} else {
		engine.TLogger.ETime.Disabled = show[0]
	}
}
...
func (engine *Engine) Logger() core.ILogger {
	return engine.TLogger.Base.Logger
}
...
func (engine *Engine) quoteColumn(keyName string) string {
	...
	keyName = strings.Replace(keyName, ",", engine.quote(","), -1)
	keyName = strings.Replace(keyName, ".", engine.quote("."), -1)

	return engine.quote(keyName)
}
...
func (engine *Engine) SetAllDB(fn func(*DB) error) error {
	for _, db := range engine.masterDB {
		if err := fn(db); err != nil {
			return err
		}
	}
	for _, db := range engine.slaveDB {
		if err := fn(db); err != nil {
			return err
		}
	}
...
func (engine *Engine) SetMaxOpenConns(conns int) {
	engine.SetAllDB(func(db *DB) error {
...
func (engine *Engine) DB(args ...bool) *core.DB {
	// read
	if len(args) > 0 && args[0] {
		return engine.SelectReadDB()
	}

	// write
	return engine.SelectWriteDB()
}

// SelectReadDB .
func (engine *Engine) SelectReadDB() *core.DB {
	slaveLength := len(engine.slaveDB)
	if slaveLength > 0 {
		var index int
		if slaveLength > 1 {
			index = rand.Intn(slaveLength - 1)
		}
		return engine.slaveDB[index]
	}
	return engine.SelectWriteDB()
}

// SelectWriteDB .
func (engine *Engine) SelectWriteDB() *core.DB {
	masterLength := len(engine.masterDB)
	if masterLength > 0 {
		var index int
		if masterLength > 1 {
			index = rand.Intn(masterLength - 1)
		}
		return engine.masterDB[index]
	}
	return engine.masterDB[0]
}
...
func (engine *Engine) Close() error {
	return engine.SetAllDB(func(db *DB) error {
		return db.Close()
	})
}

// Ping tests if database is alive
func (engine *Engine) Ping(args ...bool) error {
	session := engine.NewSession()
	defer session.Close()
...
func (engine *Engine) logSQLQueryTime(sqlStr string, args []interface{}, executionBlock func() (*core.Stmt, *core.Rows, error)) (*core.Stmt, *core.Rows, error) {
	if engine.showSQL && engine.showExecTime {
		...
		if len(args) > 0 {
			...
			engine.logger.Infof("[sql] %s - took: %vs", sqlStr, execDuration.Seconds())
		}
		return stmt, res, err
	} else {
...
func (engine *Engine) dumpAll(w io.Writer, tp ...core.DbType) error {
	...
	_, err = io.WriteString(w, fmt.Sprintf("/*Generated by dbx v%s %s*/\n\n",
	...
	for i, table := range tables {
		...
		rows, err := db.Query("SELECT * FROM " + engine.Quote(engine.tbName(table)))
		if err != nil {
			return err
		}
...
func (engine *Engine) dumpTables(tables []*core.Table, w io.Writer, tp ...core.DbType) error {
	...
	_, err := io.WriteString(w, fmt.Sprintf("/*Generated by dbx v%s %s, from %s to %s*/\n\n",
...
func (engine *Engine) mapType(v reflect.Value, args ...*core.Relation) *core.Table {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		ormTagStr := tag.Get(engine.DbxTagIdentifier)
		var col *core.Column
